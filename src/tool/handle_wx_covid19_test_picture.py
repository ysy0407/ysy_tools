...
# 房间列表
ROOM_NUMBER = [104, 106, 108, 109, 111, 113, 204, 206, 208, 209, 211, 213, 301, 304, 306, 308, 309, 311, 313, 401, 404, 406, 408, 409, 411, 413, 501, 504, 506, 508, 509, 511, 513, 601, 604, 606, 608, 609, 611, 613, 701, 704, 706, 708, 709, 711, 713, 801, 804, 806, 808, 809, 811, 813, 901, 904, 906, 908, 909, 911, 913, 1002, 1004, 1006, 1008, 1009, 1011, 1013, 1101, 1104, 1106, 1108, 1109, 1111, 1113, 1201, 1204, 1206, 1208, 1209, 1211, 1213, 1301, 1304, 1306, 1308, 1309, 1311, 1313, 1401, 1405, 1406, 1408, 1409, 1411, 1413, 1501, 1505, 1506, 1508, 1509, 1511, 1513, 1601, 1604, 1606, 1608, 1610, 1611, 1613, 1701, 1704, 1708, 1709, 1710]
# 昵称替换
NICKNAME_REPLACE_DICT = {'30弄2号': '', '30-2-': '', '陈赛平': '1309', '黄丽云': '204', '匡长铭': '211', '沈永康': '508', '兰兰': '508', '石建兰': '608', '王学廉': '1008', '梧桐佬': '811', '笑咪咪': '1209', '许樑': '813', '勋': '613', '映山红': '808', '张菊香': '206', '张忠清': '209', '朱明华': '1002', '殷克忠': '803'}
...
class HandleAntigenPicture(object):
    ...
    def __init__(self, handle_base_dir, result_excel_name='antigen_result.xlsx') -> None:
        ...
        self.result_xlsx.init_header(self.HEADER_ROW, {'B': IMAGE_CELL_SIZE[0]}, header_row_index=3)
    ...
    def save_to_excel(self, pic_name_list=None):
        ...
        self.result_xlsx.worksheet.cell(1, 1, '未发送图片的房间号：' + str(no_pic_room_number_list))
        self.result_xlsx.worksheet.cell(2, 1, '房间号不在列表中：' + str(not_in_room_number_list))
        for i, (room_number, pic_path) in enumerate(valid_pic_path_list):
            ...
            self.result_xlsx.add_image('B' + str(i+4), pic_path, IMAGE_SIZE)
...
if __name__ == '__main__':
    save_wx_picture(r'C:\data\wx-get-picture\KangYuanJieGuo\test', '30弄2号楼业主群', 100)
    handleAntigenPicture = HandleAntigenPicture(r'C:\data\wx-get-picture\KangYuanJieGuo\20220419', result_excel_name='30弄2号楼-抗原统计-20220419.xlsx')
